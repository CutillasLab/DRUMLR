...
GenerateEMSR <- function(df_input = NULL,
  ...
  df_response <-
    df_response[rownames(df_response) %in% col_cell_lines,]
  ...
  .LimmaEMSR <- function(i){
    ...
    sensitive_cells <-
      colnames(df_sens)[fm.up[grid[i, "Var1"]][[1]]]
    resistant_cells <-
      colnames(df_res)[fm.do[grid[i, "Var2"]][[1]]]
    ...
    sens_cell <-
      DRUMLR::RemoveRepeatNo(sensitive_cells)
    res_cell <-
      DRUMLR::RemoveRepeatNo(resistant_cells)
    ...
    if (ncol(df1) <= 3 |
      ...
      out <-
        data.frame("folds" = rep(0, each = nrow(df1)),
      ...
      colnames(out) <-
        paste(i, c("folds", "pvalue"), sep = ".")

    } else if (t.test(df_response[sens_cell, ], df_response[res_cell, ])$p.value >
               0.05) {
      ...
      sensitivity <-
        c(2, 1)[as.factor(unclass(colnames(dfall) %in% sens_cellname_fold))]

      sens_cellname_fold <-
        sens_cellname_fold %>% paste(collapse = ";")
      res_cellname_fold <-
        res_cellname_fold %>% paste(collapse = ";")
  ...
  EMSR_analysis <-
    list(
      "df_folds" = data.frame(EMSR_analysis[, grepl(".folds", colnames(EMSR_analysis), fixed = T), drop = F]),
  ...
  fold_groups <-
    list(
      "sensitive" = lapply(
        fm.up,
        FUN = function(x) {
          colnames(df_sens)[x] %>% paste(collapse = ";")
        }
      ) %>% unlist() %>% data.frame(),
      "resistant" = lapply(
        fm.do,
        FUN = function(x) {
          colnames(df_res)[x] %>% paste(collapse = ";")
        }
      ) %>% unlist() %>% data.frame()
    )
  fold_groups <-
    merge.data.frame(fold_groups$sensitive,
                     fold_groups$resistant,
                     by = "row.names",
                     all = T)
  fold_groups <- data.frame(rownames(fold_groups), fold_groups)
  colnames(fold_groups) <-
    c("fold_group_number", "repeat", "sensitive", "resistant")

  grid <- data.frame(rownames(grid), data.frame(grid))
  colnames(grid) <-
    c("repeat",
      "fold_group_number_sensitive",
      "fold_group_number_resistant")
  ...
  n_increase <- apply(df_folds, 1, function(x)
    sum(x == "u"))
  n_decrease <- apply(df_folds, 1, function(x)
    sum(x == "d"))

  #get average p value
  mean_p <-
    apply(
      EMSR_analysis$df_pvalues,
      1,
      FUN = function(x) {
        mean(x, na.rm = T)
      }
  ...
  df_folds[df_folds >= fold.cut.off] <- "u"
  df_folds[df_folds <= -(fold.cut.off)] <- "d"
  n_increase_filt <- apply(df_folds, 1, function(x)
    sum(x == "u"))
  n_decrease_filt <- apply(df_folds, 1, function(x)
    sum(x == "d"))
