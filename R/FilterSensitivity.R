...
FilterSensitivity <- function(sensitivity_database = NULL,
                             ...
                             cell_lines = NULL,
                             tissue_type = c("haematopoietic_and_lymphoid_tissue", "liver", "oesophagus"),
                             iqr_tolerance = 0.04,
                             n_coverage = 0.8){
  require(dplyr)

  if(is.null(cell_annotations)){cell_annotations <- DRUMLR:::PDBcellinfo %>% data.frame()}
  if(is.null(sensitivity_database)){sensitivity_database <- DRUMLR:::PDBaac %>% data.frame()}
  if(is.null(cell_lines)){cell_lines <- colnames(DRUMLR:::PDBphos)}
  ...
  #change names needed before internal database update
  #cell_annotations$cell.name <- make.names(cell_annotations$cell.name, unique = T)
  #sensitivity_database$cell.name <- make.names(sensitivity_database$cell.name, unique = T)
  ...
  #filter sensitvity by cell lines

  sensitivity_database <- sensitivity_database[sensitivity_database$cell.name %in% cell_names,]
